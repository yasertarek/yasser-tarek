...
<body>
    ...
    <div class="container">
        <form class="form" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <h1 class="text-center">Calculate Your Grade</h1>
            <div class="mb-3">
                <label for="physics" class="form-label">Physics</label>
                <input type="number" class="form-control form-outline" id="physics" name="physics" min="0" max="100">
            </div>
            <div class="mb-3">
                <label for="chemistry" class="form-label">Chemistry</label>
                <input type="number" class="form-control form-outline" id="chemistry" name="chemistry" min="0" max="100">
            </div>
            <div class="mb-3">
                <label for="biology" class="form-label">Biology</label>
                <input type="number" class="form-control form-outline" id="biology" name="biology" min="0" max="100">
            </div>
            <div class="mb-3">
                <label for="math" class="form-label">Mathematics</label>
                <input type="number" class="form-control form-outline" id="math" name="math" min="0" max="100">
            </div>
            <div class="mb-3">
                <label for="computer" class="form-label">Computer</label>
                <input type="number" class="form-control form-outline" id="computer" name="computer" min="0" max="100">
            </div>
            <button type="submit" class="btn btn-primary">Calculate</button>
            <?php
            // Check if user came through POST Method and entered all numbers
            if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['physics'] && $_POST['chemistry'] && $_POST['biology'] && $_POST['math'] && $_POST['computer']) {
                $degree = ($_POST['physics'] + $_POST['chemistry'] + $_POST['biology'] + $_POST['math'] + $_POST['computer']) / 5;
                $grade = '';
                switch ($degree) {
                    case $degree >= 90:
                        $grade = 'A';
                ...
                echo "Your Grade is: " . $grade . " and your degree is: " . $degree . "%";
            }
            ?>
        </form>
    </div>
